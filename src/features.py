...
from .config import ROLLING_WINDOW, TRAIN_TEST_SPLIT_SEASON, LABEL_MAP, FEATURE_COLUMNS
...
def create_h2h_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    For each match, calculates the average points earned by each team
    in their previous head-to-head encounters.

    Args:
        df: DataFrame with match data

    Returns:
        DataFrame with h2h_avg_points_home and h2h_avg_points_away columns
    """
    df = df.sort_values("kickoff_datetime").reset_index(drop=True)

    df["h2h_avg_points_home"] = 1.0
    df["h2h_avg_points_away"] = 1.0
    ...
    for i, row in df.iterrows():
        ...
        for _, past_game in past_games.iterrows():
            # Calculate points for each team in the past game
            ...
            # Assign points based on which team was home/away
        ...
        # Calculate average points from H2H history
...
def create_rest_days(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        if home_team in last_kickoff:
            delta = (kickoff_time - last_kickoff[home_team]).total_seconds() / (
                24 * 3600
            )
            rest_days_home.append(delta)
        else:
            rest_days_home.append(7.0)
        last_kickoff[home_team] = kickoff_time

        # Calculate away team rest days
        if away_team in last_kickoff:
            delta = (kickoff_time - last_kickoff[away_team]).total_seconds() / (
                24 * 3600
            )
            rest_days_away.append(delta)
        else:
            rest_days_away.append(7.0)
...
def prepare_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["outcome_binary"] = df["outcome_label"].apply(
        lambda x: "H_or_D" if x in ["H", "D"] else x
    )
...
def split_train_test(
    df: pd.DataFrame, split_season: int = TRAIN_TEST_SPLIT_SEASON
